    ...
    <div className="flex-1 flex flex-col min-h-0 bg-surface-950">
      <div className="px-6 py-4 border-b border-surface-800 flex-shrink-0">
        ...
        <p className="text-sm text-surface-500 mt-0.5">
          {recentFiles.length} recently opened files
        </p>
      </div>
...
function RecentItem({
  ...
  return (
    <div
      ...
      onClick={onOpen}
    >
      <span className="w-6 text-xs text-surface-600 text-right flex-shrink-0">
        {index + 1}
      </span>
      ...
      <div className="hidden sm:block text-xs text-surface-600 font-mono flex-shrink-0">
        .{file.format}
      </div>
...
function CategoryIcon({ category }: { category: MediaFile["category"] }) {
  ...
  return (
    <span className={cn("text-xs font-bold font-mono", colors[category])}>
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-surface-950">
      <div className="px-6 py-4 border-b border-surface-800 flex-shrink-0">
        ...
        <p className="text-sm text-surface-500 mt-0.5">
          {favoriteFiles.length} favorite files
        </p>
      </div>
              ...
              <div className="aspect-video bg-surface-800 overflow-hidden">
                {file.category === "image" && file.url ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center">
                    <span className="text-2xl font-mono text-surface-600">
                      .{file.format}
                    </span>
                  </div>
                )}
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center gap-4 text-center p-8 bg-surface-950">
      ...
      <div>
        ...
        <p className="text-surface-500 text-sm mt-1 max-w-xs leading-relaxed">
          {description}
        </p>
      </div>
    </div>
