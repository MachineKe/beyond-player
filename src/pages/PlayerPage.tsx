...
import { ArrowLeft, Info, MoreHorizontal, Heart, Plus, Share2 } from 'lucide-react';
    ...
    <div className="flex-1 flex flex-col min-h-0 bg-surface-950 relative">
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-surface-900/80 backdrop-blur border-b border-surface-800 flex-shrink-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          {currentFile.metadata.artist && (
            <p className="text-xs text-surface-500 truncate">{currentFile.metadata.artist}</p>
          )}
        </div>
        <div className="flex items-center gap-1">
          <button
            ...
            className={cn(
              ...
              isFav ? 'text-error-400 hover:text-error-300' : 'text-surface-400 hover:text-white hover:bg-surface-800'
            )}
      ...
      <div className="flex-1 flex min-h-0">
        <div className="flex-1 min-w-0 min-h-0 flex flex-col">
          ...
          {category === 'image' && (
            <ImageViewer src={currentFile.url} alt={currentFile.name} className="flex-1" />
          )}
          ...
          {category === 'document' && (format === 'xlsx' || format === 'csv') && (
            <SpreadsheetViewer src={currentFile.url} className="flex-1" />
          )}
          {category === 'document' && ['txt', 'json', 'xml', 'html'].includes(format) && (
            <TextViewer src={currentFile.url} format={format} className="flex-1" />
          )}
          {category === 'document' && !['pdf', 'md', 'xlsx', 'csv', 'txt', 'json', 'xml', 'html'].includes(format) && (
        ...
        {/* Metadata panel */}
        {infoOpen && (
          <MetadataPanel file={currentFile} onClose={() => setInfoOpen(false)} />
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center gap-4 text-surface-500 p-8 text-center">
      <div className="w-16 h-16 rounded-2xl bg-surface-800 flex items-center justify-center">
        <span className="text-2xl font-mono font-bold text-surface-400">.{format}</span>
      </div>
      <div>
        <p className="text-white font-medium">{name}</p>
        <p className="text-sm mt-1">Preview not available for .{format} files</p>
      </div>
    </div>
  );
}
